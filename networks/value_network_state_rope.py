...
class MultiHeadDotProductAttention(Module):
  ...
  def __call__(self,
    ...
    n = query.shape[-3]
    causal_mask_actions=jnp.diag(jnp.ones((n,), dtype=bool), k=0)[None,None,:,:]
    ...
    # query=_positional_embeddings.apply_rope(query,positions=jnp.arange(cache_k.shape[-3],cache_k.shape[-3]+query.shape[-3]), base_frequency=10000)
    # key=_positional_embeddings.apply_rope(key,positions=jnp.arange(key.shape[-3]), base_frequency=10000)
...
class transformer_layer(nn.Module):
    ...
    def __call__(self, values_keys: jnp.ndarray, queries: jnp.ndarray, cache_mask: jnp.ndarray, cache_k: Optional[jnp.ndarray] = None, cache_v: Optional[jnp.ndarray] = None):
        ...
        attention = self.attention1(inputs_kv=values_keys, inputs_q=queries_n, cache_mask=cache_mask, cache_k=cache_k, cache_v=cache_v)
...
class ValueNetworkStateRope(nn.Module):
    ...
    - output: jnp.ndarray same shape as x (logits over value bins)
    ...
    post_attention_norm: bool = False  # Apply norm after attention residual
    post_mlp_norm: bool = False  # Apply norm after MLP residual (after dense2)
    num_bins: int = 51  # number of bins for distributional value prediction
    def setup(self):


        self.embedding = nn.Dense(self.out_features, name='embedding')
        self.out_proj = nn.Dense(self.num_bins, name='out_proj')
    ...
    def __call__(self,cache_mask, cache_k, cache_v,state ) -> jnp.ndarray:


        seq = self.embedding(state)

        # run through transformer layers created in setup
        for i in range(self.num_layers):
            layer = getattr(self, f"layer_{i}")
            seq = layer(values_keys=seq, queries=seq, cache_mask=cache_mask, cache_k=cache_k[:,i], cache_v=cache_v[:,i])
        ...
        return seq  # remove state token before returning output
