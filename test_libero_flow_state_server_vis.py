...
from visualization.visualization_dual import RealtimeAgentVisualizerDual
...
def start_libero_worker(task_id: int = 1):
    ...
    try:
        libero_process = subprocess.Popen(
            ["/home/reytuag/miniconda3/envs/libero/bin/python", "-u", "libero_worker.py", str(task_id)],
...
def main():
    ...
    if libero_worker_started:
        try:
            ...
            # Step 5: Save trajectory video
            print("\n[5/6] Saving trajectory video...")
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            video_filename = f"trajectory_task{task_id}_{timestamp}.mp4"
            video_path = os.path.join("LIBERO", video_filename) if os.path.exists("LIBERO") else video_filename
            try:
