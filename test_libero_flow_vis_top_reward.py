...
def test_policy_in_environment_closed_loop(
    ...
    for replan_idx in range(num_replans):
        ...
        task_prompt_reward = f"Agent view: <start_of_image> Eye in hand view: <start_of_image>.The above images shows a robot manipulation that completes the following task:: {effective_task_instruction}. Decide whether the above statement is True or not. The answer is: "
        reward_probs = sampler.get_top_reward(
                task_prompt_reward,
                images=images_stacked,
                rng=_rng,)
