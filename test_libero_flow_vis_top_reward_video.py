...
def create_video_frame_with_attention(agentview_image: np.ndarray, eye_in_hand_image: np.ndarray, 
...
def _render_prob_curve_image(True_probs, width, height=120):
    """
    Render a probability curve plot as a numpy RGB image using matplotlib.
    
    Args:
        True_probs: List of P(True) values so far (one per replan)
    ...
    ax.set_title('Task Completion Probability', fontsize=8, fontweight='bold')
    ax.set_xlabel('Replan', fontsize=7)
    ax.set_ylabel('Prob', fontsize=7)
    ax.set_ylim(-0.05, 1.05)
    ax.grid(True, alpha=0.3)
    ax.tick_params(labelsize=6)
    
    if True_probs and len(True_probs) > 0:
        x = list(range(1, len(True_probs) + 1))
        ax.plot(x, True_probs, 'r-o', label='P(True)', markersize=3, linewidth=1.2)
        ax.set_xlim(0.5, max(len(True_probs) + 0.5, 2))
        ax.set_xticks(x)
    else:
        ax.set_xlim(0, 1)
...
def save_trajectory_video(trajectory: Dict, task_description: str, output_path: str, fps: int = 10):
    """
    Save trajectory as a video with concatenated dual-view images, task descriptor,
    and a probability curve showing P(True) over replans.
    
    Args:
        trajectory: Dictionary containing 'agentview_observations', 'eye_in_hand_observations',
                    and optionally 'True_probs', 'replans'
    ...
    True_probs_all = trajectory.get('True_probs', [])
    
    if len(agentview_obs) == 0 or len(eye_in_hand_obs) == 0:
        print("[warning] No observations to save")
        return
    
    # Build replan end-frame boundaries
    replans_info = trajectory.get('replans', [])
    replan_end_frames = []
    cumulative = 1  # frame 0 is initial obs
    for rp in replans_info:
        cumulative += rp.get('steps_executed', 16)
        replan_end_frames.append(cumulative)
    ...
    # Chart height
    chart_height = 120
    total_height = frame_h + chart_height
    
    # Initialize video writer
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(output_path, fourcc, fps, (frame_w, total_height))
    ...
    for i in range(num_frames):
        ...
        # Determine cumulative probs for this frame
        num_probs_to_show = 0
        if i >= 1 and len(True_probs_all) > 0:
            num_probs_to_show = 1
        for k in range(1, len(replan_end_frames)):
            if i >= replan_end_frames[k - 1] and k < len(True_probs_all):
                num_probs_to_show = k + 1
        
        current_True = True_probs_all[:num_probs_to_show] if num_probs_to_show > 0 else []
        
        # Render probability chart and stack below the main frame
        chart_img = _render_prob_curve_image(current_True, frame_w, chart_height)
        combined = np.concatenate([frame, chart_img], axis=0)
...
def save_trajectory_video_with_attention(
    ...
    """
    Save trajectory as a video with dual camera views, attention heatmaps,
    and a task-completion probability curve, similar to the real-time visualization layout.
    
    Args:
        trajectory: Dictionary containing 'agentview_observations', 'eye_in_hand_observations',
                    'robot_states', 'rewards', 'attention_weights' (list of per-step attention dicts),
                    'True_probs' (per-replan probability lists), and 'replans' info
        task_description: Task description to display in video
        output_path: Path to save the video file
    ...
    # Build a mapping from frame index to number of replans completed so far.
    # Frame 0 is the initial observation (before any replan).
    # Replans info gives us how many steps each replan executed.
    replans_info = trajectory.get('replans', [])
    # Compute cumulative step boundaries: frame indices where each replan ends
    replan_end_frames = []
    cumulative = 1  # frame 0 is initial obs, actions start at frame 1
    for rp in replans_info:
        cumulative += rp.get('steps_executed', 16)
        replan_end_frames.append(cumulative)
    ...
    for i in range(num_frames):
        ...
        # Determine how many replans have completed by frame i
        num_replans_done = 0
        for end_frame in replan_end_frames:
            if i >= end_frame:
                num_replans_done += 1
            else:
                break
        # For frames during a replan (after replan start but before end),
        # include that replan's probability too (it was measured at replan start)
        num_probs_to_show = min(num_replans_done + (1 if i >= 1 else 0), len(True_probs_all))
        # But more simply: replan k starts at frame replan_end_frames[k-1]+1 (or 1 for k=0)
        # and its probability is measured right at the start. So if frame i >= start of replan k,
        # show probs 0..k.
        # Recompute: replan k starts at cumulative_start[k]
        num_probs_to_show = 0
        # replan 0 starts at frame 1
        if i >= 1 and len(True_probs_all) > 0:
            num_probs_to_show = 1
        for k in range(1, len(replan_end_frames)):
            if i >= replan_end_frames[k - 1] and k < len(True_probs_all):
        ...
        current_True = True_probs_all[:num_probs_to_show] if num_probs_to_show > 0 else None
...
def test_policy_in_environment_closed_loop(
    ...
    trajectory = {
        ...
        'True_probs': [],   # True (capitalized) probability at each replan
        'prob_replan_indices': [],  # replan index for each probability measurement
    }
    
    total_steps = 0
    rng = jax.random.PRNGKey(0)
    agentview_history = []  # Collect agent view images across replans for reward prompt
    ...
    for replan_idx in range(num_replans):
        ...
        # Collect current agent view into history for reward prompt
        agentview_history.append(agentview_arr)
        
        # Build reward prompt with one <start_of_image> per historical agent view
        image_tokens = " ".join(["<start_of_image>"] * len(agentview_history))
        task_prompt_reward = f"{image_tokens} The above video shows a robot manipulation trajectory that completes the following task: {effective_task_instruction}.  Decide whether the above statement is True or not. The answer is: "
        
        # Stack all historical agent views: (1, num_history, H, W, C)
        images_stacked_history = np.stack(agentview_history, axis=0)[None, ...]
        images_stacked_history = jnp.array(images_stacked_history)
        
        reward_probs = sampler_bis.get_top_reward(
                task_prompt_reward,
                images=images_stacked_history,
                rng=_rng,)
        
        # Store the True probability for this replan
        true_prob = reward_probs['true_prob']
        True_prob = reward_probs['True_prob']
        trajectory['True_probs'].append(True_prob)
        trajectory['prob_replan_indices'].append(replan_idx)
        print(f"    Reward probs: true={true_prob:.4f}, True={True_prob:.4f}")
        ...
        for action_idx in range(steps_per_replan):
            ...
            if visualizer is not None:
                visualizer.update(
                    ...
                    True_probs=list(trajectory['True_probs']),
                )
